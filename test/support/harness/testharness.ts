...
// Minimal debug logger — gated on DEBUG_WPT so harness noise stays off by
// default. Avoids a logging dependency in a standalone package.
const logger = {
	debug(strings: TemplateStringsArray, ...values: unknown[]): void {
		if (!process.env.DEBUG_WPT) return;
